...
class FAQViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """Override list method to support language selection via ?lang= parameter."""
...
def homepage(request):
    """Render homepage based on user role (Superuser can edit, others can only view)."""
    selected_language = request.GET.get('lang', 'en')  # Default to English
...
@login_required
def add_faq(request):
    """Allow superuser to add new FAQs."""
...
def edit_faq(request, faq_id):
    """Allow superuser to edit existing FAQs."""
    if not request.user.is_superuser:
        return redirect('homepage')
...
def delete_faq(request, faq_id):
    """Allow superuser to delete existing FAQs."""
    if not request.user.is_superuser:
        return redirect('homepage')
...
def signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            return redirect('homepage')  # Redirect after signup
    else:
        form = SignUpForm()
